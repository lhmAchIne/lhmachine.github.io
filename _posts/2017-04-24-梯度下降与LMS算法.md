...
<center>$$\theta_j=\theta_j-\alpha \frac\partial{\partial\theta_j}J(\theta)$$，j=0,1,……,n（对所有j同时更新）</center>
...
<center>$$\theta_j=\theta_j-\alpha(h_\theta(\vec x)-y)x_j$$</center>
...
<center>$$\theta_j=\theta_j-\alpha\sum_{i=1}^m(h_\theta(x^{(i)})-y^{(i)})x_j^{(i)}，j=0,1,...,n$$</center>
