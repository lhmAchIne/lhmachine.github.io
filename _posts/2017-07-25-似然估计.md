...
$$ p(y^{(i)}|\vec x^{(i)};\vec \theta)=\frac1{\sqrt{2\pi}\sigma}exp(-\frac{(y^{(i)}-\vec \theta^T\vec x^{(i)})^2}{2\sigma^2}) $$

符号”$ p(y^{(i)}|x^{(i)};\vec \theta) $“表示$y^{(i)}$在给定$ \vec x^{(i)} $和参数$ \vec \theta $下的分布。需要注意

的是，由于$ \vec \theta $不是一个随机变量，所以我们不应该以$ \theta $为条件。我们也可将上式记为，$ y^{(i)}|\vec x^{(i)};\vec \theta \thicksim \mathcal N(\vec \theta^T\vec x^{(i)},\sigma^2) $
